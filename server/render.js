...
const FONT_PRESETS = {
  ...
  lato: { display: "'Lato', var(--font-body)", body: "'Lato', sans-serif" },
  nunito: { display: "'Nunito', sans-serif", body: "'Nunito', sans-serif" },
  editorial: { display: "'Merriweather', serif", body: "'Merriweather', serif" },
...
// de seguranca do preset de fonte acima): peso dos titulos (site_font_weight) e escala do tamanho
...
export function layout({
  ...
  // Ficou só contorno/cor neutra (dourado do site) a princípio; pedido novamente em 10/09/2026
  // pra ficar colorido de verdade — agora usa o gradiente oficial da marca (amarelo/laranja/
  // rosa/roxo/azul), igual o YouTube e o Google já são coloridos aqui do lado.
  const instagramIcon = `<svg width="14" height="14" viewBox="0 0 24 24" style="flex-shrink:0;"><defs><radialGradient id="igGradFooter" cx="30%" cy="107%" r="150%"><stop offset="0" stop-color="#fdf497"/><stop offset="0.05" stop-color="#fdf497"/><stop offset="0.45" stop-color="#fd5949"/><stop offset="0.6" stop-color="#d6249f"/><stop offset="0.9" stop-color="#285AEB"/></radialGradient></defs><path fill="url(#igGradFooter)" d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.012-3.584.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zm0 10.162a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.88 1.44 1.44 0 0 0 0-2.88z"/></svg>`;
  // Ícone do Google (o "G" colorido de sempre) pro link "Avalie-nos no Google", que aparece
  // aqui só quando o campo "Link de avaliação do Google" estiver preenchido em Configurações
  // (pedido do usuário em 04/09/2026, junto da configuração do Perfil da Empresa no Google).
  const googleIcon = `<svg width="14" height="14" viewBox="0 0 48 48" style="flex-shrink:0;"><path fill="#FFC107" d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"/><path fill="#FF3D00" d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"/><path fill="#4CAF50" d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"/><path fill="#1976D2" d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"/></svg>`;
  const socials = [
    ...
    ['Avalie-nos no Google', settings.google_review_url, googleIcon],
  ]
    .filter(([, url]) => url)
