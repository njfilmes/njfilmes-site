// Dados reais da NJFILMES (marcas, pessoas, bio, configurações, serviços) + projetos de exemplo.
// Extraído de scripts/seed.js para poder ser chamado tanto pela linha de comando (npm run seed,
// usado em desenvolvimento local) quanto por uma rota do painel administrativo (usada uma única
// vez em produção, quando não há acesso a terminal/shell no servidor de hospedagem).
// É seguro rodar quantas vezes quiser: só cria o que ainda não existe (idempotente).
import { query, queryOne, initSchema } from './db.js';
import * as Q from './queries.js';
import { slugify, uniqueSlug, parseVideoUrl } from './util.js';
...
export async function runSeed() {
  ...
  print('Categorias garantidas: ' + categoryNames.join(', '));
  ...
  print('Configurações atualizadas com os dados reais da NJFILMES.');

  // Marcas reais (clientes) que já aparecem no site em produção
  ...
  const brands = [
    ...
    // arquivo do código). Foi baixado como backup; falta só reenviar pelo painel /admin/marcas
    // depois da migração (arraste-e-solte, mesmo processo de sempre).
    ['Record TV', '', ''],
  ...
  print('Marcas reais garantidas: ' + brands.length);
  ...
  print('Pessoas reais garantidas: ' + people.length);
  ...
  print('Biografia atualizada com os dados reais informados.');
  ...
  for (const p of demoProjects) {
    ...
    print('Projeto de exemplo criado: ' + p.title);
  }
